...
interface KATObligationsResultsUIProps extends BaseProps {
    result: ObligationSearchResult;
    searchCriteria: ANDObligationSearchCriteria;
}

export class KATObligationsResultsUI extends EAUBaseComponent<KATObligationsResultsUIProps, any>{

    private servedObligations: Obligation[];
    private notServedObligations: Obligation[];

    constructor(props: KATObligationsResultsUIProps) {
        super(props);
    ...
    private renderObligations(obligations: Obligation[], isServed: boolean) {
        return obligations?.length > 0
            ? obligations.map((obligation, index) => {
                return <Fragment key={`${obligation.obligationIdentifier}_${index}_kat`}>
                    {
                        index == 0
                    ...
                    <tr className={isServed ? "table-warning" : ""}>
                        <td>
                            ...
                            <p className="td-text text-nowrap">
                                {
                                    obligation.additionalData.documentType == KATDocumentTypes[KATDocumentTypes.PENAL_DECREE]
                                ...
                                {
                                    isMainDocument(this.props.searchCriteria, obligation.additionalData)
                                        ? <i className="ui-icon ui-icon-state-info ml-1" title={this.getResource('GL_KAT_OBLIGATION_MAIN_DOCUMENT_I')}></i>
                                        : null
                                }
                            </p>
                        ...
                        <td>
                            <p className="th-title d-sm-none">{this.getResource('GL_VEHICLE_NUMBER_L')}</p>
                            <p className="td-text">{obligation.additionalData.vehicleNumber}</p>
                        </td>
                        <td>
                        ...
                        <td>
                            <p className="th-title d-sm-none">{this.getResource('GL_BREACH_DATE_L')}</p>
                            <p className="td-text">{formatDate(obligation.additionalData.breachDate)}</p>
                        </td>
                        ...
                        <td className="actions-td">
                            <>
                                ...
                                <ObligationsPaymentsUI obligation={obligation} searchCriteria={this.props.searchCriteria} andSourceIds={ANDSourceIds.KAT} />
                            </>
                        </td>
                    </tr>
                    {getPaymentDescription(obligation.status, obligation.paymentRequests, obligation.additionalData.documentNumber, obligation.additionalData.documentType, isServed, 11)}
                </Fragment>
            })
            : null
    }
    ...
    render() {
                ...
                <table className="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th className="w-5">{this.getResource('GL_VEHICLE_NUMBER_L')}</th>
                            <th>{this.getResource('GL_BREACH_OF_ORDER_L')}</th>
                            <th className="w-10">{this.getResource('GL_BREACH_DATE_L')}</th>
        ...
        return <fieldset className="fields-group">
            <legend>
                <h3 className="field-title">{this.getResource('GL_FIELD_OF_ACTIVITY_KAT_L')}</h3>
            </legend>
            {htmlContent}
        </fieldset>
    }
