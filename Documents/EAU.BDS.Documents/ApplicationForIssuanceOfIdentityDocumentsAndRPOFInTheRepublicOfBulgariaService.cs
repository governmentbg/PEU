...
{
    internal class ApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgariaService
        : ApplicationFormServiceBase<ApplicationForIssuanceOfIdentityDocumentsAndResidencePermitsOfForeignersInTheRepublicOfBulgaria, ApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgariaVM>
    {
        ...
        public ApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgariaService(IServiceProvider serviceProvider, INRBLDServicesClientFactory iNRBLDServicesClientFactory, IANDServicesClientFactory iANDServicesClientFactory)
        ...
        protected override string DocumentTypeUri => DocumentTypeUrisBDS.ApplicationForIssuanceOfIdentityDocumentsAndResidencePermitsOfForeignersInTheRepublicOfBulgaria;
        ...
        protected override PrintPreviewData PrintPreviewData
        {
            get
            {
                ...
                {
                    Xslt = "R-3035_ApplicationForIssuanceOfIdentityDocumentsAndResidencePermitsOfForeignersInTheRepublicOfBulgariaData.xslt",
        ...
        protected async override Task<OperationResult> InitializeApplicationFormInternalAsync(ApplicationFormInitializationRequest request, CancellationToken cancellationToken)
        {
            ...
            var form = (ApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgariaVM)request.Form;
            ...
            InitApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgaria(form, nrbldResult.Response, documentTypeIssued, service.AdditionalConfiguration["serviceCode"]);
        ...
        protected async override Task<IErrorCollection> ValidateApplicationFormInternalAsync(ApplicationFormValidationRequest request, CancellationToken cancellationToken)
        {
            ...
            var application = (ApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgariaVM)request.FormData.Form;
        ...
        public override string SignatureXpath
        {
            get
            {
                return "afidarpofthobd:ApplicationForIssuanceOfIdentityDocumentsAndResidencePermitsOfForeignersInTheRepublicOfBulgaria/afidarpofthobd:ElectronicAdministrativeServiceFooter/xmldsig:XMLDigitalSignature";
            }
        }
        ...
        public override Dictionary<string, string> SignatureXPathNamespaces
        {
            get
            {
                ...
                {
                    {"afidarpofthobd", "http://ereg.egov.bg/segment/R-3021" },
        ...
        private void InitApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgaria(ApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgariaVM application,
            ...
            string serviceCode)
        {
            ...
            var previousIdentityDocuments = responseNData.Document?.Where(d => d.DocumentType.Type.Code == DocTypeCode.DriverLicenseForForeigner || d.DocumentType.Type.Code == DocTypeCode.DriverLicense);

            Document previousIdentityDocument = null;

            if (previousIdentityDocuments != null && previousIdentityDocuments.Any())
                previousIdentityDocument = previousIdentityDocuments.OrderByDescending(x => x.IssueDate).FirstOrDefault();

            if (application.Circumstances == null && pIdentData != null)
            {
                application.Circumstances = new ApplicationForIssuanceOfIdentityDocumentsAndRPOFInTheRepublicOfBulgariaDataVM()
                {
                    ForeignIdentityBasicData = new ForeignIdentityBasicDataVM()
                    {
                        ForeignCitizenData = new ForeignCitizenDataVM()
                        {
                            ...
                            GenderName = pIdentData.Sex?.Value.ToString(),
                            Citizenship = new CitizenshipVM()
                            {
                                CountryName = pIdentData.Nationality
                            },
                            PlaceOfBirthAbroad = new PlaceOfBirthAbroad() { CountryName = responseNData?.PersonData?.BirthPlace }
                        },
                    ...
                    PermanentAddress = responseNData.Address?.id == AddresType.PermanentАddress ? new PersonAddress()
                    {
                        ...
                        Floor = responseNData.Address?.Floor?.ToString()
                    } : null,
                    PresentAddress = responseNData.Address?.id == AddresType.CurrentАddress ? new PersonAddress()
                    {
                        DistrictGRAOName = responseNData.Address?.District?.Value,
                        DistrictGRAOCode = responseNData.Address?.District != null ? responseNData.Address.District.Code?.PadLeft(2, '0') : null,
                        MunicipalityGRAOName = responseNData.Address?.Municipality?.Value,
                    ...
                    PreviousIdentityDocument = previousIdentityDocument == null ? null : new IdentityDocumentForeignCitizenBasicData()
                    {
                        IdentityDocumentType = IdentityDocumentType.DrivingLicense,
                        IdentitityExpireDate = previousIdentityDocument?.ExpiryDate,
                        IdentitityIssueDate = previousIdentityDocument?.IssueDate,
                        IdentityIssuer = previousIdentityDocument?.Issuer,
                        IdentityNumber = previousIdentityDocument?.Number
                    },
