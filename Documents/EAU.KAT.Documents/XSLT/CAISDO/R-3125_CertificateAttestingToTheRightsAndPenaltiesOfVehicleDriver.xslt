          ...
          <tr>
            <td colspan ="2">
              Настоящето да послужи&#160;<xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2001'">за Служебна бележка
                </xsl:when>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2002'">за Постъпване на работа
                </xsl:when>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2003'">пред	Застрахователя
                </xsl:when>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2004'">пред	Медицинските органи
                </xsl:when>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2005'">пред	Съдебните власти
                </xsl:when>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2006'">пред  Консултски отдел
                </xsl:when>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2007'">за	Лична информация
                </xsl:when>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="CAEFRIID:ANDCertificateReason = '2008'">за Преквалификация
                </xsl:when>
              </xsl:choose>
            </td>
          </tr>
