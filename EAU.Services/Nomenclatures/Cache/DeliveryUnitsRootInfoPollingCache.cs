...
{
    public class DeliveryUnitsRootInfoPollingCache : DataCacheItems<CachedDataInfo<List<UnitInfo>>, string>, IDeliveryUnitsRootInfoCache
    {
        ...
        public DeliveryUnitsRootInfoPollingCache(IWAISIntegrationServiceClientsFactory waisIntegrationServiceClientsFactory,
            IPollingCacheInfrastructure pollingCacheInfrastructure,
            IServices services,
            ILogger<DeliveryUnitsRootInfoPollingCache> logger)
        {
        ...
        protected override IDataCacheItem<CachedDataInfo<List<UnitInfo>>> CreateCacheItem(string key)
        {
            return new PollingDataCacheItem<List<UnitInfo>>(_logger, _pollingCacheInfrastructure, async (etag, cancellationToken) =>
            {
                ...
                if (currentService != null)
                {
                    ...
                    var data = await _waisIntegrationServiceClientsFactory.GetNomenclaturesClient().GetServiceDeliveryRootUnitsAsync(currentService.SunauServiceUri, serviceTermTypesSearch, CancellationToken.None);
