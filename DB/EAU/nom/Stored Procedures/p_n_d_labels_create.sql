...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_labels_create]
	@p_label_id		INT OUT,
	@p_code			NVARCHAR(200),
...
BEGIN
	...
	IF ( @p_code IS NULL OR 
	     @p_value IS NULL) -- PARAMETER ERROR
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[n_d_labels_create]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		SELECT @p_label_id = NEXT VALUE FOR [nom].[seq_n_d_labels];
     ...
     VALUES
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[n_d_labels_create]';
			RETURN -1;
		END;
