...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_service_declarations_search]
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_service_declarations_search]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  ...
	  -- ===============================================================================
		...
		SELECT [service_id]
			  ,[delcaration_id]			  
		  FROM [nom].[n_d_service_declarations]
		 WHERE [is_last] = 1 
		...
		IF @p_delcaration_id IS NOT NULL
		BEGIN
			SET @v_statement = @v_statement + N' 
		...
		BEGIN
			...
			EXEC sp_executeSQL @v_statement_count,
				...
				@p_delcaration_id	 = @p_delcaration_id,
