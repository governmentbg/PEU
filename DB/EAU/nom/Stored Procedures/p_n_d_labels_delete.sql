...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_labels_delete]
	@p_label_id		INT
...
BEGIN
	...
	IF (@p_label_id IS NULL) -- PARAMETER ERROR
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_labels_delete]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		UPDATE [nom].[n_d_labels]
		   SET [is_last]				= 1
			  ,[deactivation_ver_id]	= [label_ver_id]
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_labels_delete]';
			RETURN -1;
		END;
