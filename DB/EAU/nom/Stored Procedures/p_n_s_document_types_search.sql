...
-- =============================================
CREATE PROCEDURE [nom].[p_n_s_document_types_search]
	@p_ids				NVARCHAR(MAX),
	@p_start_index		INT,
	@p_page_size		INT,
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_s_document_types_search]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  ...
	  -- ===============================================================================
		...
		WHERE tablename = '[nom].[n_d_labels]' OR tablename = '[nom].[n_s_document_types]';
		...
		SELECT [doc_type_id]
			  ...
			  ,[uri]
			  ,[type_id]			  
		  FROM [nom].[n_s_document_types]
		 WHERE 1 = 1';

		IF @p_ids IS NOT NULL
		BEGIN
			SET @v_statement = @v_statement + N' 
		   AND [doc_type_id] in (SELECT * FROM @v_ids_table)
		';
		END  		
