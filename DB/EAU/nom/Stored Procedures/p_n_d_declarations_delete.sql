...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_declarations_delete]
...
BEGIN
	...
	IF (@p_delcaration_id IS NULL) -- PARAMETER ERROR
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_declarations_delete]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		UPDATE [nom].[n_d_declarations]
		   SET [is_last]							= 1
			  ,[deactivation_ver_id]				= [declaration_ver_id]			  
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_declarations_delete]';
			RETURN -1;
		END;
