...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_service_groups_delete]
	@p_group_id		INT
AS
BEGIN
	...
	IF (@p_group_id IS NULL) -- PARAMETER ERROR
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_service_groups_delete]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		UPDATE [nom].[n_d_service_groups]
		   SET [is_last]				= 1
			  ,[deactivation_ver_id]	= [group_ver_id]	  
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_service_groups_delete]';
			RETURN -1;
		END;
