...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_service_groups_i18n_update]
	@p_group_id				INT,
	@p_language_id			INT,
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_service_groups_i18n_update]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		UPDATE [nom].[n_d_service_groups_i18n]
		   SET [is_last] = 0
			  ,[deactivation_ver_id] = @v_group_i18n_ver_id		  
     ...
     VALUES
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_service_groups_i18n_update]';
			RETURN -1;
		END;
