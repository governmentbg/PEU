...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_service_groups_create]
	@p_group_id			INT OUT,
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_service_groups_create]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		SELECT @p_group_id = NEXT VALUE FOR [nom].[seq_n_d_service_groups];
     ...
     VALUES
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_service_groups_create]';
			RETURN -1;
		END;
