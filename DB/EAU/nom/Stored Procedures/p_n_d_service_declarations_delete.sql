...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_service_declarations_delete]
	@p_service_id	INT,
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_service_declarations_delete]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		UPDATE [nom].[n_d_service_declarations]
		   SET [is_last]					= 1
			  ,[deactivation_ver_id]		= [service_declaration_ver_id]
			  ,[updated_by]					= @v_curr_user_id
			  ,[updated_on]					= @v_Date
		 WHERE [service_id] = @p_service_id
		   AND [delcaration_id]	= @p_delcaration_id
		   AND [is_last] = 1
		   AND [deactivation_ver_id] IS NULL; 

		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_service_declarations_delete]';
			RETURN -1;
		END;
