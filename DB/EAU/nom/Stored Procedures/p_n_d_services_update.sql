...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_services_update]
	@p_service_id										INT,
	@p_group_id											INT,
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_services_update]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		UPDATE [nom].[n_d_services]
		   SET [is_last]				= 0
			  ,[deactivation_ver_id]	= @v_service_ver_id
     ...
     VALUES
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_services_update]';
			RETURN -1;
		END;
