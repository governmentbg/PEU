...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_declarations_search]
	@p_ids				NVARCHAR(MAX),
	@p_start_index		INT,
	@p_page_size		INT,
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_declarations_search]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  ...
	  -- ===============================================================================
		...
		WHERE tablename = '[nom].[n_d_declarations]';
		...
		BEGIN
		   ...
		   AND [delcaration_id] in (SELECT * FROM @v_ids_table)
		';
		END  		
