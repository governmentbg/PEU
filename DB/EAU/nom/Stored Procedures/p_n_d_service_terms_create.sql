...
-- =============================================
CREATE PROCEDURE [nom].[p_n_d_service_terms_create]
	@p_service_term_id		INT OUT,
	@p_service_id			INT,
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[nom].[p_n_d_service_terms_create]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		SELECT @p_service_term_id = NEXT VALUE FOR [nom].[seq_n_d_service_terms];
     ...
     VALUES
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[nom].[p_n_d_service_terms_create]';
			RETURN -1;
		END;
