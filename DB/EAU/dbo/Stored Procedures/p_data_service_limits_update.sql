...
-- =============================================
CREATE PROCEDURE [dbo].[p_data_service_limits_update]
...
BEGIN
	...
	IF (@p_service_code IS NULL OR 
		@p_requests_interval IS NULL OR 
		@p_requests_number IS NULL OR
		@p_status IS NULL) -- PARAMETER ERROR
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[dbo].[p_data_service_limits_update]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		-- ===============================================================================		
		DECLARE @v_curr_user_id INT, @v_date DATETIMEOFFSET(3), @v_service_limit_ver_id BIGINT;
		SET @v_Date = [dbo].[f_sys_get_time]();

		EXEC [dbo].[p_sys_get_current_user] @p_userid_out = @v_curr_user_id OUTPUT;
		EXEC [dbo].[p_versions_get_next] @p_version_id = @v_service_limit_ver_id OUTPUT;
		...
		INSERT INTO [dbo].[data_service_limits]
           ([service_limit_id]
		...
		SELECT
           ...
           ,@v_curr_user_id 
           ,@v_date			
		FROM [dbo].[data_service_limits]
		WHERE [service_code] = @p_service_code
		AND [deactivation_ver_id] = @v_service_limit_ver_id;



		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -16, '[dbo].[p_data_service_limits_update]';
			RETURN -1;
		END;
