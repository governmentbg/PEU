...
-- =============================================
CREATE PROCEDURE [dbo].[p_data_service_user_limits_search]
	@p_user_limit_ids			NVARCHAR(MAX),
...
BEGIN
	...
	BEGIN
	  ...
	  -- ===============================================================================
		...
		WHERE tablename = '[dbo].[data_service_user_limits]'
		...
		IF @p_user_limit_ids IS NOT NULL AND rtrim(ltrim(@p_user_limit_ids)) <> ''
		BEGIN
			INSERT INTO 
				@v_ids_table
			SELECT rtrim(ltrim(item)) FROM [dbo].[f_sys_split_string_to_int_numbers](@p_user_limit_ids, ',');
		END
		...
		SET @v_statement = N'SELECT 
					t.[user_limit_id]
					  ...
					  ,t.[requests_number]
					  ,t.[status]
					FROM dbo.[data_service_user_limits] t 
                    WHERE [is_last] = 1 AND [deactivation_ver_id] IS NULL';

		IF @p_user_limit_ids IS NOT NULL
		BEGIN
			SET @v_statement = @v_statement + N' AND t.user_limit_id in (SELECT * FROM @v_ids_table)
		';
		END
  
  		IF @p_service_limit_id IS NOT NULL
		BEGIN
			SET @v_statement = @v_statement + N' AND t.service_limit_id = @p_service_limit_id
		';
		END
		...
		BEGIN
			...
			EXEC sp_executeSQL @v_statement_count,
				...
				@p_service_limit_id     = @p_service_limit_id,
				@p_user_id				= @p_user_id,
				@p_status				= @p_status,				
		...
		SET @v_statement = @v_statement + N' ORDER BY t.user_limit_id desc
