...
-- =============================================
CREATE PROCEDURE [audit].[p_n_s_object_types_search]
...
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[audit].[p_n_s_object_types_search]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  ...
	  -- ===============================================================================
		...
		WHERE tablename = '[audit].[n_s_object_types]';
		...
		SET @v_statement = N'
			SELECT a.[object_type_id]
				  ,a.[name]
				  ,a.[description]
			  FROM [audit].[n_s_object_types] a
			 WHERE 1 = 1 ';

		IF @p_object_type_id IS NOT NULL
		BEGIN
			SET @v_statement += N' 
		   AND a.[object_type_id] = @p_object_type_id';
		END  	
