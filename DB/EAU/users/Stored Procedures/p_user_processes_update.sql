...
-- =============================================
CREATE PROCEDURE [users].[p_user_processes_update]
	@p_process_id			INT,
	@p_status				TINYINT
AS
BEGIN
	...
	BEGIN
	  EXEC [dbo].[p_sys_raise_dberror] -1, '[p_user_processes_update]';
	  RETURN -1;
	END
	ELSE
	BEGIN
	  BEGIN TRY
		...
		IF @@ROWCOUNT <> 1
		BEGIN
			EXEC [dbo].[p_sys_raise_dberror] -3, '[p_user_processes_update]';
			RETURN -1;
		END
