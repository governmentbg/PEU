...
{
    /// <summary>
    /// Клас, капсулиращ работата по извикването на процедурите от базата данни, свързани с номенклатура на държавите
    /// </summary>
    internal class CountryDataContext : BaseDataContext
    {
        public CountryDataContext(IDbContext dbContext) : base(dbContext)
        {
        }

        public async Task<(IEnumerable<Country> reader, int? count, DateTime? p_last_updated_on)> SearchAsync(
           int? p_start_index,
           int? p_page_size,
           ...
           CancellationToken cancellationToken)
        {
            ...
            var res = await _dbContext.SPQueryAsync<Country>("[nom].[p_n_s_countries_search]", parameters, cancellationToken);
