...
{
    /// <summary>
    /// Клас, капсулиращ работата по извикването на процедурите от базата данни, свързани с ЕКАТТЕ
    /// </summary>
    internal class EkatteDataContext : BaseDataContext
    {
        public EkatteDataContext(IDbContext dbContext) : base(dbContext)
        {
        }

        public async Task<(IEnumerable<Ekatte> reader, int? count, DateTime? p_last_updated_on)> SearchAsync(
          int? p_start_index,
          int? p_page_size,
          ...
          CancellationToken cancellationToken)
        {
            ...
            var res = await _dbContext.SPQueryAsync<Ekatte>("[nom].[p_n_s_ekatte_search]", parameters, cancellationToken);
