...
{
    /// <summary>
    /// Клас, капсулиращ работата по извикването на процедурите от базата данни, свързани с начините на предаване.
    /// </summary>
    internal class DeliveryChannelDataContext : BaseDataContext
    {
        public DeliveryChannelDataContext(IDbContext dbContext) : base(dbContext)
        {
        }

        public async Task<(IEnumerable<DeliveryChannel> reader, int? count, DateTime? p_last_updated_on)> SearchAsync(
            string p_ids,
            int? p_start_index,
            int? p_page_size,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var res = await _dbContext.SPQueryAsync<DeliveryChannel>("[nom].[p_n_s_delivery_channels_search]", parameters, cancellationToken);
