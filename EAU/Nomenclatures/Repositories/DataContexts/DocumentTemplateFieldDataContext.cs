...
{
    /// <summary>
    /// Клас, капсулиращ работата по извикването на процедурите от базата данни, свързани с полетата в шаблон за документ
    /// </summary>
    internal class DocumentTemplateFieldDataContext : BaseDataContext
    {
        public DocumentTemplateFieldDataContext(IDbContext dbContext) : base(dbContext)
        {
        }

        public async Task<(IEnumerable<DocumentTemplateField> reader, int? count, DateTime? p_last_updated_on)> SearchAsync(
            int? p_start_index,
            int? p_page_size,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var res = (await _dbContext.SPQueryAsync<DocumentTemplateField>("[nom].[p_n_s_document_template_fields_search]", parameters, cancellationToken));
